...
export default function SocialFeedClient({ videos, articles, tweets }: Props) {
  ...
  const showYT = platform === 'all' || platform === 'youtube';
  const showTW = platform === 'all' || platform === 'twitter';
  const showAR = platform === 'all' || platform === 'articles';
  ...
  return (
    <>
        ...
        {PLATFORMS.map(({ id, label }) => (
          <button key={id} onClick={() => setPlatform(id)}
            className={`font-condensed font-bold text-xs tracking-widest uppercase px-5 py-2.5 rounded-full transition-all ${
              platform === id ? 'bg-[#F5A623] text-black' : 'bg-[#1A1A1A] text-[#8A8A8A] hover:bg-[#242424] hover:text-white'
            }`}>
            {label}
          </button>
        ))}
      ...
      <div className="space-y-12">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-display text-white text-2xl tracking-wider">YOUTUBE</h2>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {videos.map(video => (
                <a key={video.id} href={video.url} target="_blank" rel="noopener noreferrer" className="group block bg-[#111111] rounded-lg overflow-hidden card-lift">
                  ...
                  <div className="p-3">
                    <h3 className="font-condensed font-semibold text-white text-xs leading-snug line-clamp-2 group-hover:text-[#F5A623] transition-colors">{video.title}</h3>
                    <p className="text-[#8A8A8A] text-[10px] font-condensed mt-1">{formatRelativeDate(video.publishedAt)}</p>
                  </div>
                </a>
              ))}
            </div>
        ...
        {showTW && tweets.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-black border border-[#333] rounded flex items-center justify-center">
                  <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                  </svg>
                </div>
                <h2 className="font-display text-white text-2xl tracking-wider">X / TWITTER</h2>
              </div>
        ...
        {showAR && articles.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-[#F5A623] rounded flex items-center justify-center">
                  <span className="font-display text-black text-sm">68</span>
                </div>
                <h2 className="font-display text-white text-2xl tracking-wider">ARTICLES</h2>
              </div>
