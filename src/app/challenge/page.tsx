...
const CONF_COLORS: Record<string,string> = {"Big Ten":"#003087","Big 12":"#00338D","SEC":"#004B8D","Big East":"#1B4D8E","ACC":"#003865"};
...
function StatBar({label, value, max, color="#F5A623"}: {label:string, value:number, max:number, color?:string}) {
...
function PlayerCard({player, team, slot, onRemove, draggable, onDragStart}: {player:any, team:string, slot:string, onRemove?:()=>void, draggable?:boolean, onDragStart?:()=>void}) {
  return (
    <div draggable={draggable} onDragStart={onDragStart}
      style={{background:"#111",border:"1px solid #222",borderRadius:14,padding:"16px",cursor:draggable?"grab":"default",userSelect:"none"}}>
      ...
      {player.hasStats ? (
        <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:12,textAlign:"center",background:"#0D0D0D",borderRadius:8,padding:"10px 8px"}}>
            {([["PPG",player.ppg],["RPG",player.rpg],["APG",player.apg],["SPG",player.spg],["BPG",player.bpg]] as [string,number][]).map(([l,v])=>(
              <div key={l}>
...
function PickCard({player, onClick}: {player:any, onClick:()=>void}) {
...
export default function App() {
  ...
  const [remaining, setRemaining] = useState<any[]>([...TEAMS]);
  const [currentTeam, setCurrentTeam] = useState<any>(null);
  const [squad, setSquad] = useState<(any|null)[]>(Array(6).fill(null));
  const [spinDisplay, setSpinDisplay] = useState<any>(null);
  ...
  const spinForTeam = (forSlot: number|null = null) => {
  ...
  const simulate = async () => {
    ...
    const squadStr = squad.filter(Boolean).map((p:any) =>
          ...
          <div>
            <div style={{background:"#111",border:"1px solid #1A1A1A",borderRadius:14,padding:"1.5rem",marginBottom:"1.2rem"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:"1.2rem"}}>
                {([["🎰","Spin","Get a random major conf team"],["🏀","Pick","PG · SG · SF · PF · C · Bench"],["🏆","Simulate","Full season + March Madness"]] as [string,string,string][]).map(([icon,t,d])=>(
          ...
          <div>
            <div style={{background:"#111",border:`2px solid ${gc(result.grade)}`,borderRadius:14,padding:"1.5rem",marginBottom:"1rem",textAlign:"center"}}>
              ...
              <div style={{display:"flex",justifyContent:"center",gap:28,marginBottom:16}}>
                {([["RECORD",result.record,"#F5A623"],["GRADE",result.grade,gc(result.grade)],...(result.seed?[["SEED",`#${result.seed}`,"#60a5fa"]]:[])] as [string,string,string][])
                  .map(([l,v,c])=>(
            ...
            {!result.madetournament && (
              <div style={{background:"#111",border:"1px solid #1A1A1A",borderRadius:14,padding:"1.5rem",marginBottom:"1rem",textAlign:"center"}}>
                ...
                <div style={{color:"#444",fontSize:14,marginTop:6}}>Your squad didn't make the cut.</div>
              </div>
            )}
            {result.mvp && (
