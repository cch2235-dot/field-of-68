...
export const metadata: Metadata = { title: 'About', description: 'About The Field of 68 Media Network.' };
const TEAM = [
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      <div className="mb-12 max-w-3xl">
        <div className="flex items-center gap-3 mb-4"><div className="w-1 h-7 bg-[#F5A623] rounded-full" /><h1 className="font-display text-white text-4xl md:text-5xl tracking-wider leading-none">ABOUT US</h1></div>
        <p className="text-[#8A8A8A] font-condensed text-xl leading-relaxed">Led by Jeff Goodman and Rob Dauster, two of the biggest names in the college basketball media industry, the Field of 68 Media Network is the premier destination for college basketball insight, analysis and access.</p>
      </div>

      {/* Show photos */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
        <div className="relative h-64 rounded-xl overflow-hidden">
          <Image src="/photos/show-photo-1.jpg" alt="Field of 68 After Dark" fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
            <p className="text-white font-condensed font-bold text-lg">Field of 68 After Dark — Live</p>
          </div>
        </div>
        <div className="relative h-64 rounded-xl overflow-hidden">
          <Image src="/photos/show-photo-2.jpg" alt="Field of 68 at ACC Kickoff" fill className="object-cover" sizes="(max-width: 768px) 100vw, 50vw" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
      ...
      {/* Mission */}
      <div className="bg-[#111111] rounded-2xl p-8 md:p-12 mb-12 border border-[#1A1A1A] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-[#8A8A8A] font-condensed text-lg leading-relaxed mb-4">The Field of 68 was built to be the definitive destination for college basketball fans who want more than surface-level coverage. We go deeper, report harder, and aren't afraid to take a stand.</p>
          <p className="text-[#8A8A8A] font-condensed text-lg leading-relaxed">From breaking transfer portal news to in-depth bracket analysis, from live YouTube shows to a daily newsletter — The Field of 68 is everywhere college basketball lives.</p>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
        {[{stat:'75K+',label:'YouTube Subscribers'},{stat:'154K+',label:'Twitter Followers'},{stat:'49K+',label:'Instagram Followers'},{stat:'11',label:'Original Shows'}].map(({stat,label}) => (
      ...
      {/* Team */}
      <div className="mb-12">
        <div className="flex items-center gap-3 mb-6"><div className="w-1 h-7 bg-[#F5A623] rounded-full" /><h2 className="font-display text-white text-3xl tracking-wider leading-none">CO-FOUNDERS</h2></div>
          ...
          {TEAM.map(member => (
            <div key={member.name} className="bg-[#111111] border border-[#1A1A1A] rounded-xl p-6 hover:border-[#F5A623]/40 transition-all">
              <div className="w-14 h-14 rounded-full bg-[#F5A623]/20 flex items-center justify-center mb-4">
                <span className="font-display text-[#F5A623] text-2xl tracking-widest">{member.name.split(' ').map(n => n[0]).join('')}</span>
              </div>
              ...
              <a href={`https://twitter.com/${member.twitter}`} target="_blank" rel="noopener noreferrer" className="text-[#8A8A8A] hover:text-white font-condensed text-xs transition-colors">@{member.twitter} →</a>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[#111111] border border-[#1A1A1A] rounded-2xl p-8 mb-12">
        <div className="flex items-center gap-3 mb-6"><div className="w-1 h-7 bg-[#F5A623] rounded-full" /><h2 className="font-display text-white text-3xl tracking-wider leading-none">WHERE TO FIND US</h2></div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            <a key={p.name} href={p.href} target="_blank" rel="noopener noreferrer" className="bg-[#1A1A1A] hover:bg-[#242424] rounded-lg p-4 transition-colors group">
      ...
      <div className="text-center"><Link href="/contact" className="inline-flex items-center gap-2 bg-[#F5A623] hover:bg-[#FFBE4D] text-black font-display text-xl tracking-widest uppercase px-10 py-4 rounded-lg transition-colors">Partner With Us</Link></div>
    </div>
  );
}
