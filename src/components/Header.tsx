      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          <Link href="/" className="flex-shrink-0 flex items-center">
            <Image src="/logo.png" alt="Field of 68" width={110} height={44}
            ...
            {NAV_LINKS.map(({ href, label, external }) => {
              ...
              if (external) return (
                <a key={href} href={href} target="_blank" rel="noopener noreferrer"
                  className="font-condensed font-bold text-xs tracking-widest uppercase px-3 py-2 rounded transition-colors text-[#2B2B2B] hover:bg-black/10">
                  {label}
                </a>
              );
              return (
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden p-2" aria-label="Menu">
            <div className="w-5 h-0.5 bg-[#2B2B2B] mb-1.5" style={{ transform: isOpen ? 'translateY(8px) rotate(45deg)' : '' }} />
            <div className="w-5 h-0.5 bg-[#2B2B2B] mb-1.5" style={{ opacity: isOpen ? 0 : 1 }} />
            <div className="w-5 h-0.5 bg-[#2B2B2B]" style={{ transform: isOpen ? 'translateY(-8px) rotate(-45deg)' : '' }} />
          </button>
        </div>
      </div>
